...
def run_tests():
    ...
    assert test_car.fuel == 0  # Test default
    test_car = Car("car :D", fuel=10)
...
def format_as_sentence(phrase):
    """
    Formate a phrase as a sentence
    >>> format_as_sentence('hello')
    'Hello.'
    >>> format_as_sentence('It is an ex parrot.')
    'It is an ex parrot.'
    >>> format_as_sentence('I refuse to add a monty python reference')
    'I refuse to add a monty python reference.'
    """
    return f"{phrase[0].upper()}{phrase[1:]}{'.' if phrase[-1] != '.' else ''}"
    # Works but surely there are better ways? (not to mention readability etc.)
